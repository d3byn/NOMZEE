    ...
    <div style={{ overflowX:'hidden' }}>
      ...
      <section style={{ position:'relative', minHeight:'100vh', display:'flex', alignItems:'center', padding:'120px 0 80px', overflow:'hidden' }}>
        <div style={{ position:'absolute', inset:0, pointerEvents:'none' }}>
          ...
          <div style={{ position:'absolute', inset:0, backgroundImage:'linear-gradient(rgba(128,128,128,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(128,128,128,0.04) 1px,transparent 1px)', backgroundSize:'48px 48px' }}/>
        </div>

        <div className="page-container" style={{ position:'relative', zIndex:2, maxWidth:700 }}>
          <h1 className="animate-fadeUp" style={{ fontSize:'clamp(44px,6.5vw,76px)', fontWeight:800, lineHeight:1.06, letterSpacing:'-0.03em', marginBottom:18, animationDelay:'0.1s', color:'var(--text)' }}>
          ...
          {/* Search */}
          <form className="animate-fadeUp" style={{ display:'flex', alignItems:'center', background:'var(--surface)', border:'1.5px solid var(--border)', borderRadius:16, padding:7, maxWidth:580, marginBottom:36, animationDelay:'0.2s' }}
          ...
          >
            ...
            />
            <button type="submit" style={{ width:42, height:42, background:'var(--primary)', borderRadius:10, display:'flex', alignItems:'center', justifyContent:'center', color:'white', border:'none', cursor:'pointer', flexShrink:0 }}>
      ...
      <section style={{ padding:'72px 0' }}>
        <div className="page-container">
          <div style={{ textAlign:'center', marginBottom:48 }}>
            <h2 style={{ fontSize:34, fontWeight:800, marginBottom:10, color:'var(--text)' }}>What we offer</h2>
          ...
          {featuredFoods.length === 0 ? (
            <div style={{ textAlign:'center', padding:'60px 24px', background:'var(--surface)', border:'1px dashed var(--border)', borderRadius:20 }}>
              ...
              <h3 style={{ fontSize:20, fontWeight:700, marginBottom:10, color:'var(--text)' }}>No items listed yet</h3>
      ...
      {!user && (
        <section style={{ padding:'72px 0' }}>
          <div className="page-container">
            <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:24, padding:'52px 48px', display:'flex', justifyContent:'space-between', alignItems:'center', gap:32, flexWrap:'wrap', position:'relative', overflow:'hidden' }}>
              ...
              <div>
                <h2 style={{ fontSize:32, fontWeight:800, marginBottom:10, color:'var(--text)' }}>Ready to order?</h2>
...
function FeaturedCard({ food, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:18, overflow:'hidden', cursor:'pointer', transition:'all 0.25s' }}
      onMouseEnter={e => { e.currentTarget.style.transform='translateY(-5px)'; e.currentTarget.style.borderColor='rgba(255,77,0,0.25)'; e.currentTarget.style.boxShadow='var(--card-shadow)' }}
    ...
    >
      ...
      <div style={{ padding:'16px' }}>
        <div style={{ fontWeight:700, fontSize:15, marginBottom:4, color:'var(--text)' }}>{food.name}</div>
