...
function ImageUploader({ onChange, resetKey }) {
  const [dragOver,   setDragOver]   = useState(false)
  const [preview,    setPreview]    = useState(null)
  const [processing, setProcessing] = useState(false)
  const fileRef = useRef()

  useEffect(() => { setPreview(null); if (fileRef.current) fileRef.current.value = '' }, [resetKey])
  ...
  return (
    <div>
      <div
        ...
        style={{ border:`2px dashed ${dragOver ? 'var(--primary)' : preview ? 'var(--success)' : 'var(--border)'}`, borderRadius:14, background: dragOver ? 'rgba(255,77,0,0.04)' : 'var(--bg2)', cursor:preview?'default':'pointer', transition:'all 0.25s', overflow:'hidden', position:'relative', width:'100%', aspectRatio:'1/1', maxWidth:280, display:'flex', alignItems:'center', justifyContent:'center' }}
      >
        {processing ? (
          ...
          <>
            ...
            <button type="button" onClick={e => { e.stopPropagation(); clearImage() }}
              style={{ position:'absolute', top:10, right:10, width:34, height:34, borderRadius:'50%', background:'rgba(0,0,0,0.65)', border:'1.5px solid rgba(255,255,255,0.25)', color:'white', fontSize:17, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}
...
export default function DashboardPage({ user, onFoodsUpdated, toast }) {
  ...
  useEffect(() => { fetchMyRestaurants().then(list => fetchMyFoods(list)) }, [])
  ...
  const addRestaurant = async (e) => {
    ...
    try {
      ...
      const saved = res.data?.restaurant || res.data
      const msg   = res.data?.message || `Restaurant "${saved.name}" submitted for approval!`
      toast(msg, 'success')
  ...
  const addFood = async (e) => {
    ...
    const selRest = myRestaurants.find(r => String(r.id) === selectedRestId)
    if (selRest && selRest.status !== 'APPROVED') {
      toast(`Restaurant is "${selRest.status || 'PENDING'}" — wait for admin approval before adding food.`, 'error')
      return
    }
    setFLoading(true)
    try {
      ...
      const payload  = { name:fForm.name.trim(), description:fForm.description.trim(), price:parseFloat(fForm.price), imageUrl, restaurant:{ id:rid } }
  ...
  /* ── Styles ── */
  const inp = { background:'var(--input-bg)', border:'1.5px solid var(--border)', borderRadius:11, padding:'12px 14px', color:'var(--text)', fontSize:15, width:'100%', transition:'all 0.2s', fontFamily:"'DM Sans',sans-serif" }
    ...
    <div>
      <div className="page-header">
        <div className="page-container page-header-inner">
          ...
          {selectedRest && selectedRest.status === 'APPROVED' && (
      ...
      <div className="page-container" style={{ padding:'32px 24px 80px', maxWidth:860 }}>
        ...
        {tab === 'restaurant' && (
          <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:18, padding:32, animation:'fadeUp 0.3s ease' }}>
            <h3 style={{ fontSize:20, fontWeight:700, marginBottom:6, color:'var(--text)' }}>Register Your Restaurant</h3>
            <p style={{ color:'var(--text2)', fontSize:14, marginBottom:20 }}>Your restaurant will be submitted for admin approval. Once approved you can add food items.</p>

            {/* Existing restaurants with status */}
            {myRestaurants.length > 0 && (
              <div style={{ marginBottom:28 }}>
                <div style={{ fontSize:12, fontWeight:700, color:'var(--text2)', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:10 }}>Your Restaurants</div>
                {myRestaurants.map(r => {
                  const sc = r.status === 'APPROVED' ? 'var(--success)' : r.status === 'REJECTED' ? '#EF4444' : r.status === 'BLOCKED' ? '#EF4444' : 'var(--accent)'
                  const sb = r.status === 'APPROVED' ? 'rgba(34,197,94,0.08)' : r.status === 'REJECTED' || r.status === 'BLOCKED' ? 'rgba(239,68,68,0.08)' : 'rgba(255,183,0,0.08)'
                  const si = r.status === 'APPROVED' ? '✓' : r.status === 'REJECTED' ? '✕' : r.status === 'BLOCKED' ? '🚫' : '⏳'
                  return (
                    <div key={r.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px', background:sb, border:`1px solid ${sc}33`, borderRadius:12, marginBottom:8 }}>
                      <div style={{ flex:1 }}>
                        <div style={{ fontWeight:700, fontSize:14, color:'var(--text)' }}>{r.name}</div>
                        <div style={{ fontSize:12, color:'var(--text3)' }}>{r.address}</div>
                      </div>
                      <div style={{ fontSize:12, fontWeight:700, color:sc, background:sb, border:`1px solid ${sc}44`, padding:'4px 12px', borderRadius:8 }}>
                        {si} {r.status || 'PENDING'}
                      </div>
                    </div>
                  )
            ...
            <form onSubmit={addRestaurant} style={{ display:'flex', flexDirection:'column', gap:18 }}>
              ...
              <button type="submit" disabled={rLoading} style={{ alignSelf:'flex-start', background:'var(--primary)', color:'white', padding:'13px 30px', borderRadius:11, fontSize:15, fontWeight:700, fontFamily:"'Syne',sans-serif", border:'none', cursor:rLoading?'not-allowed':'pointer', opacity:rLoading?0.7:1 }}>
                {rLoading ? 'Submitting...' : '+ Submit for Approval'}
              </button>
            </form>
          </div>
        )}
        ...
        {tab === 'food' && (
          <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:18, padding:32, animation:'fadeUp 0.3s ease' }}>
            <h3 style={{ fontSize:20, fontWeight:700, marginBottom:6, color:'var(--text)' }}>Add Food Item</h3>
            <p style={{ color:'var(--text2)', fontSize:14, marginBottom:28 }}>Fill in the details and upload a photo. It will appear in the menu for customers.</p>
            ...
            <form onSubmit={addFood} style={{ display:'flex', flexDirection:'column', gap:20 }}>
                ...
                {restsLoading ? (
                  ...
                  <>
                    <select style={{ ...inp, cursor:'pointer' }} value={selectedRestId} onChange={e => setSelectedRestId(e.target.value)} onFocus={fo} onBlur={bl} required>
                      ...
                      {myRestaurants.map(r => (
                        <option key={r.id} value={String(r.id)}>{r.name} · {r.address} [{r.status || 'PENDING'}]</option>
                      ))}
                    </select>
                    {(() => {
                      const sel = myRestaurants.find(r => String(r.id) === selectedRestId)
                      if (!sel) return null
                      if (sel.status === 'APPROVED') return <div style={{ fontSize:12, color:'var(--success)', marginTop:6 }}>✓ Approved — you can add food items</div>
                      if (sel.status === 'REJECTED') return <div style={{ fontSize:12, color:'#EF4444', marginTop:6 }}>✕ Rejected by admin. Contact support.</div>
                      if (sel.status === 'BLOCKED')  return <div style={{ fontSize:12, color:'#EF4444', marginTop:6 }}>🚫 Blocked by admin.</div>
                      return <div style={{ fontSize:12, color:'var(--accent)', marginTop:6 }}>⏳ Awaiting admin approval before adding food</div>
                    })()}
                  </>
                ) : (
              ...
              {myRestaurants.length > 0 && selectedRestId && (<>
                ...
                <div><label style={lbl}>Food Image (auto-cropped to 1:1)</label><ImageUploader onChange={setImgDataUrl} resetKey={imgResetKey}/></div>
        ...
        {tab === 'list' && (
          <ManageMenuTab
            ...
            onFoodUpdated={async () => { await fetchMyFoods(); if (onFoodsUpdated) onFoodsUpdated() }}
            toast={toast}
          />
        )}
      </div>
...
function ManageMenuTab({ foods, loading, myRestaurantIds, setTab, onRefresh, onFoodUpdated, toast }) {
  const [unavailable,    setUnavailable]    = useState(() => { try { return new Set(JSON.parse(localStorage.getItem(UNAVAIL_KEY)||'[]')) } catch { return new Set() } })
  const [search,         setSearch]         = useState('')
  const [editingId,      setEditingId]      = useState(null)
  const [editForm,       setEditForm]       = useState({})
  const [editImg,        setEditImg]        = useState('')
  const [editImgKey,     setEditImgKey]     = useState(0)
  const [editLoading,    setEditLoading]    = useState(false)
  const [deleteConfirm,  setDeleteConfirm]  = useState(null)
  const [deleteLoading,  setDeleteLoading]  = useState(false)

  const myFoods  = foods.filter(f => myRestaurantIds.has(String(f.restaurant?.id ?? f.restaurantId ?? '')))
  const filtered = myFoods.filter(f => !search || f.name?.toLowerCase().includes(search.toLowerCase()) || f.description?.toLowerCase().includes(search.toLowerCase()))
  ...
  const startEdit = (f) => { setEditingId(f.id); setEditForm({ name:f.name, description:f.description||'', price:String(f.price), imageUrl:f.imageUrl||'' }); setEditImg(''); setEditImgKey(k => k+1) }
  ...
  const saveEdit = async (foodId) => {
    ...
    try {
      ...
      await axios.put(`/food/update/${foodId}`, { name:editForm.name.trim(), description:editForm.description.trim(), price:parseFloat(editForm.price), imageUrl }, { withCredentials:true })
      if (editImg) saveImage(foodId, editForm.name.trim(), editImg)
      toast('Food item updated! ✓', 'success')
      setEditingId(null); setEditImg('')
      if (onFoodUpdated) onFoodUpdated()
    } catch (err) { toast(typeof err.response?.data === 'string' ? err.response.data : 'Failed to update', 'error') }
  ...
  if (loading) return <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:18, padding:40, display:'flex', justifyContent:'center' }}><div className="loader"/></div>

  if (myFoods.length === 0) return (
    <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:18, padding:32 }}>
      <h3 style={{ fontSize:20, fontWeight:700, marginBottom:24, color:'var(--text)' }}>Manage Menu</h3>
    ...
    <>
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:18, padding:32, animation:'fadeUp 0.3s ease' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:12 }}>
          <div>
            <h3 style={{ fontSize:20, fontWeight:700, marginBottom:4, color:'var(--text)' }}>
              Manage Menu
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {filtered.map(f => {
            ...
            return (
              <div key={id} style={{ border:`1px solid ${isEditing?'rgba(255,77,0,0.35)':isOff?'rgba(239,68,68,0.25)':'var(--border)'}`, borderRadius:14, overflow:'hidden', transition:'all 0.25s', background:isEditing?'rgba(255,77,0,0.02)':isOff?'rgba(239,68,68,0.02)':'var(--bg2)' }}>
                ...
                {!isEditing && (
                  <div style={{ display:'flex', alignItems:'center', gap:14, padding:'14px 16px', opacity:isOff?0.7:1 }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontWeight:700, fontSize:15, marginBottom:2, display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', color:'var(--text)' }}>
                        {f.name}
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
                      ...
                      {/* Toggle */}
                      <div onClick={() => toggleAvailability(f.id)} title={isOff?'Mark Available':'Mark Unavailable'}
                ...
                {isEditing && (
                  <div style={{ padding:'20px' }}>
                    <div style={{ fontSize:14, fontWeight:700, color:'var(--primary)', marginBottom:16 }}>✏️ Editing: {f.name}</div>
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
                      <div>
                        ...
                        <input style={inp} value={editForm.name} onChange={e => setEditForm(p => ({ ...p, name:e.target.value }))} onFocus={fo} onBlur={bl}/>
                      </div>
                      <div>
                        ...
                        <input style={inp} type="number" min="1" value={editForm.price} onChange={e => setEditForm(p => ({ ...p, price:e.target.value }))} onFocus={fo} onBlur={bl}/>
                      </div>
                    </div>
                    <div style={{ marginBottom:14 }}>
                      ...
                      <input style={inp} value={editForm.description} onChange={e => setEditForm(p => ({ ...p, description:e.target.value }))} onFocus={fo} onBlur={bl}/>
                    </div>
                    <div style={{ marginBottom:18 }}>
                      <label style={{ fontSize:11, fontWeight:700, color:'var(--text2)', textTransform:'uppercase', letterSpacing:'0.06em', display:'block', marginBottom:6 }}>Update Image (optional)</label>
                      <ImageUploader onChange={setEditImg} resetKey={editImgKey}/>
                    </div>
                    <div style={{ display:'flex', gap:10 }}>
                      ...
                      <button onClick={() => { setEditingId(null); setEditImg('') }} style={{ background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:10, padding:'10px 20px', fontSize:14, color:'var(--text2)', cursor:'pointer', fontFamily:"'DM Sans',sans-serif" }}>Cancel</button>
                    </div>
                  </div>
                )}
              </div>
      ...
      {deleteConfirm && (
        ...
        >
          ...
          >
            ...
            <h3 style={{ fontSize:20, fontWeight:700, textAlign:'center', marginBottom:8, color:'var(--text)' }}>Delete Item?</h3>
            <p style={{ color:'var(--text2)', fontSize:14, textAlign:'center', marginBottom:28, lineHeight:1.6 }}>
              This will permanently remove <strong>{foods.find(f => f.id === deleteConfirm)?.name}</strong> from the menu and database.
            </p>
            <div style={{ display:'flex', gap:12 }}>
              <button onClick={() => !deleteLoading && setDeleteConfirm(null)} style={{ flex:1, background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:12, padding:'13px', fontSize:15, color:'var(--text2)', cursor:'pointer', fontFamily:"'DM Sans',sans-serif" }}>Cancel</button>
              <button onClick={() => confirmDelete(deleteConfirm)} disabled={deleteLoading} style={{ flex:1, background:'#EF4444', color:'white', border:'none', borderRadius:12, padding:'13px', fontSize:15, fontWeight:700, cursor:deleteLoading?'not-allowed':'pointer', fontFamily:"'Syne',sans-serif", opacity:deleteLoading?0.7:1, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
