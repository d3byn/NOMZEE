      ...
      <div className="page-container" style={{ padding:'28px 24px 80px' }}>
        ...
        {filtered.length === 0 ? (
          <div className="empty-state">
            ...
            <p>{foods.length === 0 ? "Restaurant partners haven't added their menu yet." : 'Try a different search term'}</p>
...
function FoodCard({ food, qty, setQty, onAdd, adding, canAdd, isGuest, isUnavailable, isBlockedByAdmin, delay }) {
  ...
  return (
    <div style={{ background:'var(--surface)', border:`1px solid var(--border)`, borderRadius:18, overflow:'hidden', transition:'all 0.25s', animation:`fadeUp 0.4s ease ${delay}s both`, opacity:isUnavailable?0.55:1, position:'relative' }}
      onMouseEnter={e => { if(!isUnavailable){ e.currentTarget.style.transform='translateY(-5px)'; e.currentTarget.style.boxShadow='var(--card-shadow)'; e.currentTarget.style.borderColor='rgba(255,77,0,0.2)' }}}
    ...
    >
      ...
      {/* Image */}
      <div style={{ height:'auto', aspectRatio:'1/1', background:'var(--bg3)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:68, position:'relative', overflow:'hidden' }}>
        {uploadedImg
          ? <img src={uploadedImg} alt={food.name} style={{ width:'100%', height:'100%', objectFit:'cover', filter:isUnavailable?'grayscale(60%)':'none' }}/>
          : <span style={{ filter:'drop-shadow(0 6px 12px rgba(0,0,0,0.15))' }}>{isEmoji ? food.imageUrl : fallbackEmoji}</span>
        }
        {food.restaurant?.name && (
          <div style={{ position:'absolute', bottom:10, right:10, background:'rgba(0,0,0,0.55)', color:'white', fontSize:11, fontWeight:500, padding:'3px 9px', borderRadius:6, backdropFilter:'blur(4px)' }}>
            {food.restaurant.name}
          </div>
        )}
      </div>
      ...
      <div style={{ padding:'18px' }}>
        <div style={{ fontWeight:700, fontSize:15, marginBottom:4, lineHeight:1.3, color:'var(--text)' }}>{food.name}</div>
        {food.description && (
          <div style={{ color:'var(--text2)', fontSize:13, marginBottom:8, lineHeight:1.5 }}>{food.description}</div>
        )}
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
          <span style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:20, color:'var(--text)' }}>₹{food.price}</span>
        ...
        {isUnavailable ? (
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <div style={{ display:'flex', alignItems:'center', background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:9, overflow:'hidden' }}>
              ...
              >−</button>
              <span style={{ minWidth:28, textAlign:'center', fontSize:14, fontWeight:600, color:'var(--text)' }}>{qty}</span>
              <button onClick={() => setQty(qty+1)} style={{ width:32, height:34, background:'none', border:'none', color:'var(--text2)', fontSize:17, cursor:'pointer' }}
                onMouseEnter={e => e.target.style.background='var(--border)'}
                onMouseLeave={e => e.target.style.background='none'}
              >+</button>
